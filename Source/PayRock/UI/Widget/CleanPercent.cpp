...
void UCleanPercent::SetCleanData()
{
	...
	int32 Dead = PRGS->DieMonsterCount;
	int32 Total = PRGS->TotalEnemyCount;
	...
	if (UTextBlock* CleanMessage = Cast<UTextBlock>(GetWidgetFromName(CleanTextMessage)))
	{
		...
		if (Percent>= 0.5)
		{
			CleanMessage->SetText(FText::FromString("어둠이 걷혀져 맑아진 기분이다"));
		}
		else if (Percent>= 0.8)
		{
			CleanMessage->SetText(FText::FromString("이 빌어먹을 저주를 거의 다 끝내간다"));
		}
		else if (Percent==1)
		{
			CleanMessage->SetText(FText::FromString("마을의 정중앙에 특별한 보상이 출현하였다."));
		}
	}
}
